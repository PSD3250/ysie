...
### AI 코멘트 systemInstruction + GAS 후처리 이중 차단 (커밋: 69dca23)
- **배경**: AI가 "피드백입니다" 소개 문장, "보여주셨습니다" -시- 존칭, "높은 백분위" 등 금지 표현을 프롬프트 지시에도 계속 위반
- **수정 1 (systemInstruction)**: GAS CALL_GEMINI에서 Gemini API 호출 시 `systemInstruction` 필드 추가
  - 소개/제목 문장 금지 (-인 것들 등), -시- 존칭 금지, 백분위 해석 방향, 격식체, 과도한 칭찬 금지, 영역명 한국어화 등 6개 규칙
  - 시스템 레벨 적용 → user 프롬프트보다 강력하게 적용됨
- **수정 2 (후처리)**: AI 응답 수신 후 GAS 서버에서 정규식으로 금지 표현 강제 교체
  - 소개 문장 패턴("피드백입니다", "결과입니다" 등) → 첫 줄 제거
  - -시- 존칭 패턴 8종 교체 ("보여주셨습니다→보여줍니다" 등)
- **효과**: 속도 영향 없음, 이중 차단으로 AI 위반 원천 차단
- **⚠️ GAS 재배포 필요**

### AI 코멘트 프롬프트 구조 개선 (커밋: f2b30fb)
- **수정 1**: "상위 백분위" 표현 전체 → "백분위"로 일괄 치환 (AI가 "상위=높다"로 오해 원천 차단)
- **수정 2**: 백분위 데이터 표현 명확화 → "백분위 75% = 전체 학생 중 75%가 이 학생보다 높은 점수"로 명시
- **수정 3**: 부진/매우 부진 학생(백분위 55% 초과)에 대한 "잘한 점" 지시 분기
  - 중위권 이상: 기존 "잘한 점 (2문장)" 유지
  - 중하위권 이하: "현재 성취 수준 기술" + "잘했다/높다/우수하다 절대 금지" 지시로 변경
- **효과**: 부진 학생에게 억지로 긍정적 표현 강제하는 문제 해결

### 프론트엔드 보안 전면 강화 (커밋: 1a63882 외)
- **배경**: F12 개발자 도구에서 geminiKey, adminCode, masterCode 노출 위험
- **GAS 수정**: GET_CONFIG에서 민감 키 필터링, VERIFY_CODE API 신설(서버 측 비밀번호 비교), CALL_GEMINI 프록시화(GAS가 직접 시트에서 키 읽음), UPDATE_CONFIG_KEYS API 신설
- **script.js 수정**: 민감 정보 초기화 제거, 모든 fetch에서 Content-Type 헤더 제거(CORS preflight 차단), verifyAuth 서버 경유 방식으로 전환
- **⚠️ GAS 재배포 완료**
